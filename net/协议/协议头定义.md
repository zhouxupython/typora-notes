...
![](icmp长度计算-差错报文-超时.jpg)



#### icmp-端口不可达

![](icmp长度计算-差错报文-端口不可达.jpg)
...
（1）tcp有**首部长度**字段，因为tcp首部长度 = 必须字段长度（20字节） + options长度，options长度不固定（最大40字节），所以需要指明。
...
（1）udp没有**首部长度**字段，即没有 udp头长度 字段，因为长度固定就是8
